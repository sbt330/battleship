...
void handlePlayerRequest(int player, Socket *client)
{
  ...
  if(gameLogic.inSetupMode()) {
  	printf("In setup mode.\n");
  	
    // handle the requests made during setup mode.
    // when both players have placed all of their ships, then start the game.
    if (request.op == 1)
    {
    	printf("Placing ship %d for player %d at (%d, %d)\n", request.shipType, player, request.row, request.col);
    	
    	/*
    	bool placeShip = gameLogic.placeShip(player, request.shipType, request.row, request.col, request.dir);
